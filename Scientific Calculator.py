# Muhammad Ehsan Mumtaz(Reg no 039)
# Muhammad Farooq Naeem(Reg no 036)

import tkinter as tk
from math import *
import numexpr as ne                                               #install numexpr


#Evaluate Function
def evaluate():                                                    #User defined function
    expression = input_entry.get()                                 #Input
    try:
        result = ne.evaluate(expression)                           #Solve expression by numexpr
        result_label.configure(text=result)                        #Display result in result box
    except:
        result_label.configure(text="Error")                       #Display error if conditon not met

#Clear screen Function
def clear():
    input_entry.delete(0, tk.END)                                  #delete all the entries
...
def calculate_square_root():                                      
    ...
    try:
        value = float(expression)                                  #Value in float
        result = sqrt(value)
...
root.geometry("365x450")                                          #Window size
root.title("Scientific Calculator")                               #Title
root.configure(bg='#4A4E69')                                      #Background Clour
...
main_button_frame.place(x=10, y=115)                               #Frame Placement
...
#Entry Box
input_entry = tk.Entry(in_frame, font=('Sans-serif', 16), width=34, bd=0, justify=tk.CENTER)
input_entry.pack()
